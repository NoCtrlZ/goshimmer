...
func (bconn *peeredConnection) receiveData(data []byte) {
	...
	if bconn.peer != nil {
		...
		if bconn.peer.handshakeOk {
			...
			qnode_events.Events.PeerMessageReceived.Trigger(msg)
		} else {
...
func (bconn *peeredConnection) processHandShakeOutbound(msg *qnode_events.PeerMessage) {
...
func (bconn *peeredConnection) processHandShakeInbound(msg *qnode_events.PeerMessage) {
	peerAddr := string(msg.MsgData)
