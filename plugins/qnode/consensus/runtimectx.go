...
// inputs (immutable) and outputs (mutable) to VM, accessible though through RuntimeContext interface
...
func (ctx *runtimeContext) PrevTime() time.Time {
	return time.Unix(0, ctx.stateTx.MustState().Timestamp())
}

func (ctx *runtimeContext) RequestVars() variables.Variables {
	return ctx.reqMsg.MustRequest(ctx.reqMsg.Index).Variables()
}

func (ctx *runtimeContext) StateVars() variables.Variables {
